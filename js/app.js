...
async function saveToGitHub() {
  ...
  try {
    await pushFiles([
      { path: 'data/index.json',  content: JSON.stringify(routes, null, 2) },
      { path: 'data/groups.json', content: JSON.stringify(groups, null, 2) },
    ], 'Update route index and groups');
...
// ── GitHub commit (atomic — all files in one commit) ─────────────────────────
async function pushFiles(fileList, message) {
  ...
  // 1. Get HEAD commit SHA + tree SHA
  const refRes = await fetch(`${base}/git/ref/heads/main`, { headers });
  if (!refRes.ok) throw new Error('Could not get repo ref. Check token and repo name.');
  ...
  const treeItems = await Promise.all(fileList.map(async ({ path, content, binary }) => {
    let blobContent, encoding;
    if (binary) {
      // GPX: send as base64
      blobContent = btoa(unescape(encodeURIComponent(content)));
      encoding = 'base64';
    } else {
      blobContent = content;
      encoding = 'utf-8';
...
async function confirmImport() {
  ...
  try {
    ...
    await pushFiles([
      { path: `data/gpx/${id}.gpx`, content: pendingGpxText,                    binary: true },
      { path: 'data/index.json',     content: JSON.stringify(newRoutes, null, 2) },
      { path: 'data/groups.json',    content: JSON.stringify(newGroups, null, 2) },
    ], `Add route: ${name}`);
...
async function saveToGitHub() {
  ...
  try {
    ...
    // Sequential to avoid SHA conflicts
    await pushFile({ owner, repo, headers, path:'data/index.json',  content:JSON.stringify(routes, null, 2), message:'Update route index' });
    await pushFile({ owner, repo, headers, path:'data/groups.json', content:JSON.stringify(groups, null, 2), message:'Update groups' });
...
async function pushFile({ owner, repo, headers, path, content, message }) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    // Always fetch the very latest SHA before each attempt
    let sha;
    try {
    ...
    if (res.status === 409 && attempt < 2) {
      await new Promise(r => setTimeout(r, 500 * (attempt + 1))); // brief backoff
      continue;
    }
...
async function confirmImport() {
  ...
  try {
    ...
    await pushFile({ owner, repo, headers, path:`data/gpx/${id}.gpx`, content:pendingGpxText, message:`Add GPX: ${name}` });
    await pushFile({ owner, repo, headers, path:'data/index.json',     content:JSON.stringify(newRoutes,null,2), message:`Add route: ${name}` });
    await pushFile({ owner, repo, headers, path:'data/groups.json',    content:JSON.stringify(newGroups,null,2), message:`Update groups` });
...
async function pushFile({ owner, repo, headers, path, content, message }) {
  ...
  // Always fetch fresh SHA immediately before writing
  let sha;
  try {
  ...
  // If SHA mismatch, fetch again and retry once
  if (res.status === 409) {
    try {
      const r2 = await fetch(base, { headers });
      if (r2.ok) { const d = await r2.json(); body.sha = d.sha; }
    } catch {}
